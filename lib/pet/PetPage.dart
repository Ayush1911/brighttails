...
class PetPage extends StatefulWidget {
  ...
  const PetPage({super.key, this.GetData});
...
class _PetPageState extends State<PetPage> {
  ...
  Future<List<dynamic>?> getdata() async {
    Uri url = Uri.parse(UrlRescource.ALLPETDATA);

    try {
      var response = await http.post(
        url,
        body: {'breed_id': widget.GetData}, // Send the category ID properly
      );
  ...
  void filterEvents(String query) {
    final filtered = _allEvents?.where((event) {
      final category = event["pet_name"].toString().toLowerCase();
                      ...
                      return GestureDetector(
                        ...
                        child: Card(
                          ...
                          child: Column(
                            children: [
                              Container(
                                width: 180, // ✅ Ensures uniform image height
                                height: 150, // ✅ Ensures uniform image height
                                padding: const EdgeInsets.all(2),
                                child: Image.network(
                                  img + pet["pet_image_1"].toString(),
